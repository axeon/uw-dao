...
public class ConnectionPool {
    ...
    public ConnectionPool(String poolName) {
        ...
        ConnPoolConfig config = DMConfigManager.getConnPoolConfig(this.poolName);
        // 驱动
    ...
    public Connection getConnection() {
        ...
        if (available) {
            ...
            for (int outerloop = 0; outerloop < 200; outerloop++) {
                ...
                // 超过30次找不到，报告连接耗尽信息
                if (outerloop > 30) {
                    logger.warn("-----> ConnectionPool[" + poolName + "](" + connList.size() + ") Exhausted!  Will wait and try again in loop " + outerloop);
                }
            }
        } else {
    ...
    void createConn(String reason) {
        if (connList.size() < maxConns) {
            try {
                ...
                // 获得一个封装对象
                ConnectionWrapper cw = new ConnectionWrapper(DriverManager.getConnection(dbServer, dbUsername, dbPassword), poolName);
                cw.setReadyStatus();
                connList.add(cw);
                if (logger.isTraceEnabled()) {
                    logger.trace("ConnectionPool[" + poolName + "](" + connList.size() + ") opening connection : " + cw.toString() + " " + reason);
                }
            } catch (Exception e) {
                logger.error("--->ConnectionPool[" + poolName + "](" + connList.size() + ") Attempt failed to create new connection " + reason, e);
            }
        }
    }
    ...
    public void destroy() {
        ...
        if (connList.size() > 0) {
            // bt-test successful
            String msg = "ConnectionPool[" + poolName + "] Unsafe shutdown: Had to close " + connList.size() + " active DB connections";
            logger.error(msg);
        }
