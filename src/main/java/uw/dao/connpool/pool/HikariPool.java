...
import uw.dao.connpool.HikariPoolMXBean;
import uw.dao.connpool.metrics.MetricsTrackerFactory;
import uw.dao.connpool.metrics.PoolStats;
import uw.dao.connpool.metrics.dropwizard.CodahaleHealthChecker;
import uw.dao.connpool.metrics.dropwizard.CodahaleMetricsTrackerFactory;
import uw.dao.connpool.metrics.micrometer.MicrometerMetricsTrackerFactory;
import uw.dao.connpool.util.ConcurrentBag;
import uw.dao.connpool.util.SuspendResumeLock;
import uw.dao.connpool.util.UtilityElf;
...
import static uw.dao.connpool.util.ConcurrentBag.IConcurrentBagEntry.STATE_IN_USE;
import static uw.dao.connpool.util.ConcurrentBag.IConcurrentBagEntry.STATE_NOT_IN_USE;
import static uw.dao.connpool.util.UtilityElf.*;
...
public final class HikariPool extends PoolBase implements HikariPoolMXBean, ConcurrentBag.IBagStateListener {
     ...
     * through {@link } then {@code owner} is {@code true}.
     * <p>
    ...
    private ScheduledExecutorService initializeHouseKeepingExecutorService() {
        if (config.getScheduledExecutor() == null) {
            final ThreadFactory threadFactory = Optional.ofNullable(config.getThreadFactory()).orElse(new UtilityElf.DefaultThreadFactory(poolName + " housekeeper", true));
